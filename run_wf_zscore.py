#!/usr/bin/env python3
"""Walk-forward analysis for Z-Score Mean Reversion strategy at 1h and 4h."""
...
from strategies.zscore_mr_strategy import ZscoreMrStrategy
...
def main():
    ...
    # Z-Score MR param grid (full grid from strategy)
    grid = {
    ...
    print("Z-SCORE MEAN REVERSION — 1h — Rolling 18mo IS / 3mo OOS", file=sys.stderr)
    print("=" * 80, file=sys.stderr)
    ...
    wf_1h = run_walk_forward(
        strategy_class=ZscoreMrStrategy,
        df=df_1h,
        grid=grid,
        config=config,
    ...
    # --- 4h Walk-Forward ---
    print("=" * 80, file=sys.stderr)
    print("Z-SCORE MEAN REVERSION — 4h — Rolling 18mo IS / 3mo OOS", file=sys.stderr)
    print("=" * 80, file=sys.stderr)
